        ...
        <div slot="main">
          <gz-panel @onMainResize="onResizeHeight">
            <div slot="main" class="main">
              <Table :height="tableHeight" :ref="refNames.table_ListData" size="small" :loading="listData.isLoading" :columns="listData.columns"
      ...
      <Modal :ref="refNames.detailModal" title="API详情" v-model="modalStatus" width=80 :styles="{'max-width':'500px'}" :mask-closable="false"
        ...
        <Form :ref="refNames.editForm" :model="editData" :rules="dataRule" label-position="left" :label-width="80">
          <FormItem label="接口名称" prop="APIName">
            <Input v-model="editData.APIName" placeholder="请输入API名称"></Input>
          </FormItem>
      ...
      event_buttonImportClick(event, component) {
        ...
        requestAPIList
          ...
          .then(res => {
            ...
            _this.showInfo(res.result.message);
            component.loading = false;
          })
          .catch(err => {
            component.loading = false;
